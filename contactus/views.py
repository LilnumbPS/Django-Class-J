...
def user_login(request):
    ...
    content={'form_login':form_login}
    return render(request,'contactus/login.html',content)
...
def change_password(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if not user.check_password(old_password):
                return HttpResponse('پسوود قبلی شما درست نیست')
            elif new_password1!=new_password2:
                return HttpResponse('پسووردهای شما با هم مطابقت ندارند')
            else:
    ...
    context={'form':form}
    return render(request,'contactus/changepassword.html',context)